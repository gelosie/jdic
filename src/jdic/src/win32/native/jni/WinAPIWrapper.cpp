...
JNIEXPORT jbyteArray JNICALL Java_org_jdesktop_jdic_desktop_internal_impl_WinAPIWrapper_AssocQueryString
  (JNIEnv * env, jclass cl, jbyteArray fileOrProtocal, jbyteArray verb) {
    ...
    HRESULT rc = AssocQueryString(ASSOCF_NOTRUNCATE, ASSOCSTR_COMMAND, fileOrProtocalStr, verbStr, pszOut, &pcchout);
    if (rc != S_OK) {
        result = NULL;   
    } else {
...
char * jstringToUnicode(JNIEnv * env, jstring jstr)
{
	...
	size = WideCharToMultiByte(CP_ACP, 0, (unsigned short *)jcstr, length, ustr, (length*2+1), NULL, NULL);
	if (!size)
		return NULL;
...
(JNIEnv * env, jclass obj, jobjectArray toArray, jobjectArray ccArray, jobjectArray bccArray, jstring subject, jstring body, jobjectArray attachArray)
{
    ...
    if(ulResult) {
		...
		switch (ulResult) {
			case MAPI_E_FAILURE:
				env->ThrowNew(excCls, "Logon failed: One or more unspecified errors occurred during logon");
				break;
			case MAPI_E_INSUFFICIENT_MEMORY:
				env->ThrowNew(excCls, "Logon failed: There was insufficient memory to proceed");
				break;
			case MAPI_E_LOGIN_FAILURE:
				env->ThrowNew(excCls, "Logon failed: There was no default logon, and the user failed to log on successfully when the logon dialog box was displayed");
				break;
			case MAPI_E_TOO_MANY_SESSIONS:
				env->ThrowNew(excCls, "Logon failed: The user had too many sessions open simultaneously");
				break;
			case MAPI_E_USER_ABORT:
				env->ThrowNew(excCls, "Logon failed: The user canceled the logon dialog box");
				break;
			default:
	    ...
	    if(ulResult) {
			...
			switch (ulResult) {
				case MAPI_E_AMBIGUOUS_RECIPIENT:
					env->ThrowNew(excCls, "A recipient matched more than one of the recipient descriptor structures");
					break;
				case MAPI_E_ATTACHMENT_NOT_FOUND:
					env->ThrowNew(excCls, "The specified attachment was not found");
					break;
				case MAPI_E_ATTACHMENT_OPEN_FAILURE:
					env->ThrowNew(excCls, "The specified attachment could not be opened");
					break;
				case MAPI_E_BAD_RECIPTYPE:
					env->ThrowNew(excCls, "The type of a recipient was not MAPI_TO, MAPI_CC, or MAPI_BCC");
					break;
				case MAPI_E_FAILURE:
					env->ThrowNew(excCls, "One or more unspecified errors occurred.");
					break;
				case MAPI_E_INSUFFICIENT_MEMORY:
					env->ThrowNew(excCls, "There was insufficient memory to proceed");
					break;
				case MAPI_E_INVALID_RECIPS:
					env->ThrowNew(excCls, "One or more recipients were invalid or did not resolve to any address");
					break;
				case MAPI_E_LOGIN_FAILURE:
					env->ThrowNew(excCls, "There was no default logon, and the user failed to log on successfully when the logon dialog box was displayed");
					break;
				case MAPI_E_TEXT_TOO_LARGE:
					env->ThrowNew(excCls, "The text in the message was too large");
					break;
				case MAPI_E_TOO_MANY_FILES:
					env->ThrowNew(excCls, "There were too many file attachments");
					break;
				case MAPI_E_TOO_MANY_RECIPIENTS:
					env->ThrowNew(excCls, "There were too many recipients");
					break;
				case MAPI_E_UNKNOWN_RECIPIENT:
					env->ThrowNew(excCls, "A recipient did not appear in the address list");
					break;
				case MAPI_E_USER_ABORT:
					break;
				default:
					env->ThrowNew(excCls, "One or more undefined errors occurred.");
			}
		}
	}
